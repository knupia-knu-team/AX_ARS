// ═══════════════════════════════════════════════════════════
// Call Mate AX — E-commerce Agent Workbench Mock Engine (E-commerce Only)
// ═══════════════════════════════════════════════════════════

import type { APIAnalysisResult } from '../types/shared.js';

/**
 * 이커머스 전용 상담 분류 규칙 (Fuzzy Rule Engine)
 * 
 * 카테고리 (18개):
 * - 주문 확인, 결제 오류, 중복 결제, 쿠폰/할인 적용 오류, 배송 지연, 빠른배송 지연, 배송지 변경,
 *   상품 파손, 상품 누락, 오배송, 반품 요청, 교환 요청, 환불 지연, 정기배송/구독 문의,
 *   멤버십 혜택 문의, 판매자 문의, 반복 민원, 일반 문의
 * 
 * 부서 (9개):
 * - 주문/결제팀, 배송지원팀, 반품/교환팀, 환불정산팀, 상품품질팀, 쿠폰/멤버십팀, 판매자지원팀, 민원전담팀, 일반상담팀
 */
export function runMockAnalysis(text: string): APIAnalysisResult {
  ...
  // 비-이커머스 문의 체크 (금융, 의료, 시설물 긴급안전, 도로 위험 등)
  ...
  if (isNonEcommerce) {
    return {
      ...
      normalizedText,
      mainIntent: '일반 문의',
      subIntent: '이커머스 범주 외 문의',
      detectedKeywords: ['범주 외'].filter(k => normalizedText.includes(k)),
      urgencyLevel: 'Low',
      urgencyScore: 15,
      confidenceScore: 98,
      recommendedDepartment: '일반상담팀',
      alternativeDepartments: [],
      summaryForAgent: '이커머스 고객센터의 상담 범위를 벗어난 일반 외부(안전/금융/공공 등) 문의입니다.',
      recommendedFAQ: [
        '주문번호를 모르는 고객의 문의는 어떻게 시작하나요?',
        '현금영수증이나 거래명세서 발급 문의는 어떻게 안내하나요?'
      ],
      responseGuide: '본 이커머스 지원 범위를 벗어난 내용임을 정중히 안내하고 관련 전문 대행기관으로 연결할 수 있도록 대외 연락망을 안내합니다.',
      crmCategoryCode: 'ECO-OUT-000',
      nextActions: ['이커머스 범주 외 문의 사항임을 고객에게 설명', '일반상담팀 안내 지침 전달 후 통화 종결'],
      explanation: '본 문의 사항은 금융/안전/공공 분야의 건으로, 이커머스 상담 범위를 초과하여 일반상담팀으로 분류합니다. 해당 문의는 이커머스 지원 범위 외의 내용입니다.',
      analysisSource: 'Mock Rule Engine'
    };
  }

  // 10종 대화 시나리오 1:1 고정 분기 및 정밀 매핑 (Fuzzy Rule Engine 시뮬레이션)
  
  // 1. 배송 지연 (1번 생수) - Medium (40%)
  if (normalizedText.includes('생수') || normalizedText.includes('도도착 예정이었는데')) {
    mainIntent = '배송 지연';
    subIntent = '배송 예정일 초과 지연';
    detectedKeywords = ['생수', '배송', '지연', '도착'].filter(k => normalizedText.includes(k));
    urgencyLevel = 'Medium';
    urgencyScore = 55;
    recommendedDepartment = '배송지원팀';
    alternativeDepartments = ['일반상담팀'];
    summaryForAgent = '배송 약속 기한이 경과하였으나 상품을 수령하지 못한 배송 지연 상태에 대한 불만 문의입니다. 실시간 허브 위치 추적이 필요합니다.';
    recommendedFAQ = [
    ...
    explanation = '상품 배송 및 운송장 지연 문제로 분석되어 이를 실시간 확인 가능한 배송지원팀으로 이관합니다.';
  }
  // 2. 상품 파손 교환 (2번 도자기) - Medium (40%)
  else if (normalizedText.includes('도자기') || normalizedText.includes('에어캡')) {
    mainIntent = '상품 파손';
    subIntent = '배송 상품 불량/파손';
    detectedKeywords = ['파손', '도자기', '배송', '교환'].filter(k => normalizedText.includes(k));
    urgencyLevel = 'Medium';
    urgencyScore = 60;
    recommendedDepartment = '상품품질팀';
    alternativeDepartments = ['반품/교환팀', '배송지원팀'];
